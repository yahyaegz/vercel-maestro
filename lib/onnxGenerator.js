...
export class ONNXMusicGenerator {
  ...
  async generate(numTokens = 200, temperature = 0.8, onProgress = null) {
    ...
    for (let i = 0; i < numTokens; i++) {
      // Ensure pattern is exactly 100 tokens long
      let inputPattern = [...pattern];
      while (inputPattern.length < 100) {
        inputPattern.unshift(0); // Pad with token 0 (e.g. Wait_0.0) at the beginning
      }
      if (inputPattern.length > 100) {
        inputPattern = inputPattern.slice(inputPattern.length - 100);
      }
      
      const seqLen = 100;
      
      const srcTensor = new ort.Tensor('int64', new BigInt64Array(inputPattern.map(BigInt)), [1, seqLen]);
      // The ONNX exported model uses a fixed size of 100 and no src_mask input
      const feeds = { src: srcTensor };
